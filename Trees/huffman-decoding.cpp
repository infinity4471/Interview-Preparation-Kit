...
void decode_huff_helper( node* root, string& s, int& idx ) {
    if( root->left == nullptr && root->right == nullptr ) { // leaf root
        printf("%c", root->data );
        return;
    }
    else if( s[ idx ] == '0' ) decode_huff_helper( root->left, s, ++idx );
    else decode_huff_helper( root->right, s, ++idx );
}

void decode_huff(node * root, string s) {
    int idx = 0;
    while( idx < s.length() ) decode_huff_helper( root, s, idx );
    printf("\n");
}
